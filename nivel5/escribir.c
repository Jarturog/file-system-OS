//./escribir disco "$(cat textos.txt)"
//./escribir disco "123456789"
...
int main(int argc, char const *argv[]){
    ...
    for (int i = 0; i < (sizeof(offsets) / sizeof(offsets[0])); i++){
        ...
        /*bytes = mi_read_f(ninodo, buf, offsets[i], longitud);
        if(bytes == FALLO){
            fprintf(stderr, "Error leyendo usando mi_read_f en escribir.c");
            return FALLO;
        }
