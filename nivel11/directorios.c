...
int mi_link(const char *camino1, const char *camino2) {
    ...
    int valor_de_retorno = EXITO;
    if (!camino1 || !camino2 || camino1[strlen(camino1) - 1] == '/' || camino2[strlen(camino2) - 1] == '/') { // Comprobar que las rutas no apunten a ficheros
        return FALLO;
    }
    mi_waitSem();
    while (1) { // al final hay break
        int error = buscar_entrada(camino1, &p_inodo_dir1, &p_inodo1, &p_entrada1, 0, 4);
        if  (error < 0) { // Buscar entrada de camino1
            valor_de_retorno = error;
            break;
        }
...
int mi_unlink(const char *camino) {
    ...
    while (1) { // al final hay break
        ...
        if  (error < 0) { // Buscar entrada de camino
            valor_de_retorno = error;
            break;
        }
        ...
        if (inodo.tipo == 'd' && inodo.tamEnBytesLog > 0) {
            valor_de_retorno = ERROR_DIRECTOR_NO_VACIO;
            break;
        }
