...
int main(int argc, char const *argv[]){
    ...
    if (ninodo == FALLO) {
        fprintf(stderr, ROJO "Error reservando inodo en escribir.c\n" RESET);
        return FALLO;
    }
    ...
    for (int i = 0; i < (sizeof(offsets) / sizeof(offsets[0])); i++){
        ...
        int bytes = mi_write_f(ninodo, texto, offsets[i], longitud);
        if (bytes == FALLO){
            fprintf(stderr, ROJO "Error usando mi_write_f en escribir.c\n" RESET);
            return FALLO;
        }
        ...
        if (diferentes_inodos == 1) { // sí se reservan diferentes inodos
            ...
            if (ninodo == FALLO){
                fprintf(stderr, ROJO "Error al reservar inodo en escribir.c\n" RESET);
                return FALLO;
            }
        }
    }
